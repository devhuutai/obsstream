...
.get((req, res)=>{
  saveSystemInfo()
  con.query(`SELECT * FROM noah_systeminfo WHERE id=6`,(err, result)=>{
    if(err) throw err
    const [data] = result
    const dataFormat = {
      port : data?.port,
    ...
    res.render(`${resolve('./fe/index')}`,{dataFormat:dataFormat})
...
const createStreamDirectory = (streamKey) => {
  ...
  nms.on('postPublish', (id, StreamPath, args) => {
    ...
    .on('end', () => {
      con.query(`SELECT status FROM noah_streamkey  WHERE token=${con.escape(StreamPath.split('/')[2])}`,(err, result)=>{
        if(err) throw err
        var [data] = result
        if(data?.status == 1){
          fsExtra.removeSync(streamDirectory);
        }else{
          console.log('ngu')
        }
      })
